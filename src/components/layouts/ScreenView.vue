...
<template>
    <div class="flex flex-row justify-end h-screen  ">
        <!-- Sidebar -->
        <div :class="useStoreMenu.isLoadMenu ? 'w-64 max-lg:left-0' : 'w-16 max-lg:-left-full'"
        ...
        <!-- Main Content -->
        <!-- w-[calc(100%-16rem)] -->
        <div :class="useStoreMenu.isLoadMenu ? 'ml-64 ' : 'ml-16'"
            class="  flex flex-col w-full duration-700 max-lg:ml-0 ease-in-out transition-all">
            <!-- Navbar -->
            <!-- fixed top-0 left-64 right-2 -->
            <nav class="w-full flex flex-row justify-between items-center bg-white shadow-sm z-10 py-2 px-3 ">
                <NavbarView />
            </nav>
